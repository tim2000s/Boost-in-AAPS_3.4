...
*Me, 28 days across the two changes. Switching sensitivity to fixed changed nothing I can measure. Going to a one-minute cycle moved both delivery and time below range.*
...
The other change was switching from dynamic ISF to a fixed one, and what's interesting is that I can't measure any difference at all. Time in range moved by under four points with an interval spanning zero comfortably, and so did everything else. With six days in that arm the comparison can't resolve anything smaller than about eight points, so this is underpowered rather than negative, but it's not nothing either.

It's not nothing because I went looking at this properly earlier this year across about 115 looping people and millions of decisions, and the finding was that a well-set static ISF already matches what the loop does and beats the dynamic equations, with the steepest form doing worst. The honest relationship between sensitivity and total daily dose came out gentle, roughly 1/√TDD, not the 1/TDD or 1/TDD² the equations assume.
